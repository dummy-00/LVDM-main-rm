...
class DDPM(pl.LightningModule):
    ...
    def log_images(self, batch, N=8, n_row=2, sample=True, return_keys=None, 
        ...
        if sample:
            # get denoise row
            with self.ema_scope("Plotting"):
                # 如果不需要可视化 denoising 过程，设为 False 可以显著降低显存
                if plot_denoise_rows:
                    all_samples =[]
                    all_denoise_rows =[]
                    for _ in range(N):
                        # 每次只采样 1 张，获取最终结果和中间过程
                        sample_single, denoise_row_single = self.sample(batch_size=1, return_intermediates=True)
                        all_samples.append(sample_single)
                        all_denoise_rows.append(denoise_row_single)
                    
                    samples = torch.cat(all_samples, dim=0)
                    
                    # 重新拼接中间过程: denoise_row 结构为 [img_t1, img_t2, ...]
                    num_steps = len(all_denoise_rows[0])
                    denoise_row =[]
                    for i in range(num_steps):
                        # 把第 i 步的 N 张图在 batch 维度拼接
                        step_images = torch.cat([row[i] for row in all_denoise_rows], dim=0)
                        denoise_row.append(step_images)
                else:
                    # 如果不需要 denoise row，只循环采样最终结果
                    all_samples =[]
                    for _ in range(N):
                        sample_single = self.sample(batch_size=1, return_intermediates=False)
                        all_samples.append(sample_single)
                    samples = torch.cat(all_samples, dim=0)
                    denoise_row = None
            ...
            if plot_denoise_rows and denoise_row is not None:
                log["denoise_row"] = self._get_rows_from_list(denoise_row)
...
class LatentDiffusion(DDPM):
    ...
    @torch.no_grad()
    def decode_first_stage_2DAE(self, z, decode_bs=4, return_cpu=True, **kwargs):
        b, _, t, _, _ = z.shape
