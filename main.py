...
if __name__ == "__main__":
    ...
    if opt.auto_resume:
        ...
        if os.path.exists(ckpt):
            ...
            try:
                ...
                if 'epoch' in tmp and 'global_step' in tmp:
                    e = tmp['epoch']
                    gs = tmp['global_step']
                    print(f"[INFO] Resume from epoch {e}, global step {gs}!")
                else:
                    print("[INFO] last.ckpt exist but no epoch info, starting fresh!")
                    resume = False # 强制视为不恢复
                del tmp
            except:
                try:
    ...
    try:
        ...
        ckpt_path = "/home/plj/LVDM-main/models/ae/ae_sky.ckpt"
        ...
        # 核心：必须手动清空这些字段，防止 Trainer 再次尝试加载
        config.model.load_from_checkpoint = None
        # ------------------------------------------------
        ...
        if "load_from_checkpoint" in config.model and config.model.load_from_checkpoint and not resume:
            try:
                ...
                state_dict = torch.load(config.model.load_from_checkpoint, map_location=f"cuda:{gpu_id}")
        ...
        if hasattr(trainer_opt, 'checkpoint_callback'):
            setattr(trainer_opt, 'checkpoint_callback', None)
        ...
        # 调试：打印所有 ModelCheckpoint 回调的参数
        # 调试：打印所有 ModelCheckpoint 回调的参数 (修改版)
        print("=" * 50)
        print("ModelCheckpoint callbacks found in trainer_kwargs:")
        for idx, cb in enumerate(trainer_kwargs["callbacks"]):
            if isinstance(cb, pl.callbacks.ModelCheckpoint):
                print(f"Callback {idx}: {cb}") # 直接打印对象本身，比访问属性安全
        print("=" * 50)
        ...
        print("--- 验证 Checkpoint 回调 ---")
        ckpt_found = False
        for cb in trainer.callbacks:
            if isinstance(cb, pl.callbacks.ModelCheckpoint):
                print(f"检测到 Checkpoint 回调: dirpath={cb.dirpath}, filename={cb.filename}")
                ckpt_found = True
        if not ckpt_found:
            print("警告：没有检测到任何 ModelCheckpoint 回调！CKPT 可能不会保存！")
        trainer.logdir = logdir

        if not cpu:
            gpus_val = lightning_config.trainer.gpus
            if isinstance(gpus_val, int):
                ngpu = 1  # 单个 GPU 时数量为 1
            elif isinstance(gpus_val, str):
                ngpu = len(gpus_val.strip(",").split(','))
            elif isinstance(gpus_val, (list, tuple)):
                ngpu = len(gpus_val)
            else:
                ngpu = 1
        else:
        ...
        def melk(*args, **kwargs):
            # run all checkpoint hooks
            if trainer.global_rank == 0:
                print("Summoning checkpoint (with cleanup).")
